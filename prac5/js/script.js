...
let loading = false; // Флаг, предотвращающий множественные одновременные запросы

function loadMoreArticles() {
  if (loading) return; // Предотвращение повторных запросов, пока идет загрузка

  loading = true;
  const loadMoreButton = document.getElementById('loadMore');
  loadMoreButton.disabled = true; // Деактивируем кнопку во время загрузки
  loadMoreButton.textContent = 'Загрузка...';

  fetch(`/server/response.php?par=publication&offset=${offset}&limit=${limit}`)
    .then(response => response.json())
    .then(data => {
      if (data.error) {
        console.error("Error:", data.error);
        alert("Произошла ошибка при загрузке данных."); // Или более изящная обработка ошибки
      } else {
        appendArticles(data);
        offset += limit;
        if (data.length < limit) {
          loadMoreButton.style.display = 'none'; // Скрыть кнопку, если больше нет данных
        }
      }
      loading = false;
      loadMoreButton.disabled = false;
      loadMoreButton.textContent = 'Смотреть ещё';
    })
    .catch(error => {
...
function appendArticles(articles) {
  ...
  articles.forEach(article => {
    ...
    articleElement.innerHTML = `<h3>${article.title}</h3><p>${article.text}</p>`; // Настройте разметку под ваши нужды
    articlesContainer.appendChild(articleElement);
  });
}


// Добавить кнопку "Смотреть ещё"
const loadMoreButton = document.createElement('button');
loadMoreButton.id = 'loadMore';
loadMoreButton.textContent = 'Смотреть ещё';
loadMoreButton.addEventListener('click', loadMoreArticles);

// Вставить кнопку после контейнера с элементами
const articlesContainer = document.getElementById('articles');
articlesContainer.parentNode.insertBefore(loadMoreButton, articlesContainer.nextSibling);
